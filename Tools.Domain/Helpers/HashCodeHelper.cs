...
namespace Tools.Domain.Helpers
{
    ...
    public static class HashCodeHelper
    {
        ...
        /// </summary>
        /// <remarks>Le HashCode est une valeur qui est utilisée dans la comparaison de 2 entités. Il est utilisé dans les méthodes Equals() et CompareTo().
        /// Ne pas oublier de surcharger et d'implémenter les méthodes GetHashCode(), Equals(obj), CompareTo(obj)</remarks>
        ...
        /// <summary>
        /// Obtient le HashCode d'une entité en se basant sur la présence des attributs <see cref="UniqueKeyAttribute"/> ou <see cref="UniqueByAttribute"/>
        /// </summary>
        /// <remarks>Le HashCode est une valeur qui est utilisée dans la comparaison de 2 entités. Il est utilisé dans les méthodes Equals() et CompareTo().
        /// Ne pas oublier de surcharger et d'implémenter les méthodes GetHashCode(), Equals(obj), CompareTo(obj)</remarks>
        ...
        private static int? EvaluateHashCodeWithUniqueOnClass(IEntity entity)
        {
            ...
            {
                var properties = entityType.GetProperties().Where(prop => uniqueByAttribute.PropertyNames.Contains(prop.Name)).OrderBy(prop => prop.Name).ToList();
        ...
        /// <summary>
        /// Evalue le hashcode de l'objet en sa basant sur les attributs <see cref="UniqueKeyAttribute"/> indiqués sur les propriétés
        /// </summary>
        /// <param name="entity">Objet à évaluer</param>
        /// <returns></returns>
        private static int? EvaluateHashCodeWithUniqueOnProperties(IEntity entity)
        {
            ...
            foreach (var property in properties)
            {
                var uniqueKeyAttribute = property.GetCustomAttributes(typeof(UniqueKeyAttribute), true).FirstOrDefault() as UniqueKeyAttribute;
