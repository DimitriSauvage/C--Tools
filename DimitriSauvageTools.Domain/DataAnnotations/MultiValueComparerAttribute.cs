...
namespace DimitriSauvageTools.Domain.DataAnnotations
{
    ...
    public class MultiValueComparerAttribute : ValidationAttribute
    {
        ...
        /// <param name="expectedValuesByProperties">Dictionnaire qui contient des association clé / valeur avec en clé le nom de la propriété à comparer et en valeur, la valeur attendue.</param>
        public MultiValueComparerAttribute(object valueToCompare, params object[] expectedValuesByProperties)
        {
            ...
            this.ExpectedValuesByProperties = new Dictionary<string, object>();
            for (int i = 0; i < expectedValuesByProperties.Length / 2; i++)
			{
                ExpectedValuesByProperties.Add((string)expectedValuesByProperties[2 * i], (object)expectedValuesByProperties[2 * i + 1]);
			}
        }
        #endregion
        ...
        #region Methods
        /// <summary>
        /// Détermine si les valeurs des propriétés spécifiées sont valides. Les types des propriétés doivent être du même type.
        /// </summary>
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            Type typeToCompare = validationContext.ObjectType.GetProperty(ExpectedValuesByProperties.First().Key).PropertyType;
            foreach (var otherProperty in ExpectedValuesByProperties)
            {
                ...
                if (!property.PropertyType.Equals(typeToCompare))
                    return new ValidationResult("Les types de toutes les propriétés à comparer doivent-être identiques.");
