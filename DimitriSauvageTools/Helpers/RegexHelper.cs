...
namespace DimitriSauvageTools.Helpers
{
    ...
    public static class RegexHelper
    {
        public enum RegexType
        {
            ...
            [Description(@"(BP)\s?(\d+)")]
            FrenchPostBox,
            [Description(@"(CEDEX)\s?(\d+)")]
            FrenchCedex,
            [Description(@"^([a-zA-ZáàâäãåçéèêëíìîïñóòôöõúùûüýÿæœÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ._\s-]*)\ \(((0[1-9])|([1-8][0-9])|(9[0-8])|(2A)|(2B))[0-9]{3}\)")]
            CityFullNameWithFrenchZipCode,
            [Description(@"\{(.*?)\}")]
            RefServiceLeafCategoryNameWithAlias,
        ...
        {
            return new Regex(EnumHelper.ToDescription(regexType)).IsMatch(input);
        }

        /// <summary>
        /// Recherche dans la chaine spécifiée en paramètre la première occurence trouvée qui correspond au type de regex choisi
        /// </summary>
        /// <param name="regexType"></param>
        /// <param name="input"></param>
        /// <returns></returns>
        ...
        {
            return new Regex(EnumHelper.ToDescription(regexType)).Match(input);
        }
        ...
        {
            return new Regex(EnumHelper.ToDescription(regexType)).Matches(input);
        }
        ...
        {
            if (trim)
                return new Regex(EnumHelper.ToDescription(regexType)).Replace(input, string.Empty).Trim();
            else
                return new Regex(EnumHelper.ToDescription(regexType)).Replace(input, string.Empty);
        }
        ...
        {
            return Regex.Replace(input, EnumHelper.ToDescription(regexType), replacement);
        }
